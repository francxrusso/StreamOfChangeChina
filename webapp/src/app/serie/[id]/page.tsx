...
import { hasSupabaseConfig, supabase, type PublicEpisodio, type PublicSerie } from "@/lib/supabase";
...
async function getSerieDetail(id: string) {
  ...
  try {
    ...
    if (serieError) {
      return { serie: null, episodi: [], error: serieError.message };
    }
    ...
    if (episodiError) {
      return { serie: null, episodi: [], error: episodiError.message };
    }
    ...
    return { serie: null, episodi: [], error: message };
  }
}
...
export default async function SerieDetailPage({
  ...
  const { serie, episodi, error } = await getSerieDetail(id);
